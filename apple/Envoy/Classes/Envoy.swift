...
public class Envoy {
    ...
    public enum Proxy: CustomStringConvertible {
        ...
        public func maybeModify(_ request: URLRequest) -> URLRequest {
            switch self {
            case .envoy(var url, let headers, let salt):
                ...
                if let host = request.url?.host {
                    modified.addValue(host, forHTTPHeaderField: "Host-Orig")
                }

                if let url = request.url {
                    modified.addValue(url.absoluteString, forHTTPHeaderField: "Url-Orig")
                }
    ...
    public func initialize(urls: [URL]) {
...
//        let testRequest = URLRequest(url: URL(string: "https://www.google.com/generate_204")!)
//
