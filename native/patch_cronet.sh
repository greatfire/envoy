...
# as feature
patch --fuzz=0 --no-backup-if-mismatch --forward --strip=1 --force <"$PATCH_DIR/0006-Add-as-features-and-android-preferences.patch"

patch --fuzz=0 --no-backup-if-mismatch --forward --strip=1 --force <"$PATCH_DIR/0007-Add-cmdline-switch.patch"
patch --fuzz=0 --no-backup-if-mismatch --forward --strip=1 --force <"$PATCH_DIR/0008-Set-host-header-for-http2.patch"

# FAILED
#patch --fuzz=0 --no-backup-if-mismatch --forward --strip=1 --force <"$PATCH_DIR/0009-Add-setting-page-for-envoy-url.patch"
